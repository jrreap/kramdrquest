...
let deadenemy = {name: "dead", health: 0, damage: 1, icon: "skull"}
    ...
    return (
      <Animatable.View style={styles.container} animation="fadeIn" duration="1000" iterationCount={1} easing="ease-in">
        <View style={styles.innercontainer}>
          ...
          <Animatable.Text animation="fadeInLeft" delay="4000" style={{fontWeight: "bold", fontSize: 20, paddingBottom: 10}}>Rallying the soilders...</Animatable.Text>
...
class Home extends React.Component {
  ...
  _calculateLoot(amount) {
    let dice = amount * this._rollDice();
  ...
  _createQuestQuestion() {
    let question = quests[cardindex - 1];
    let dice = this._rollDice();
    ...
    switch (card.name) {
      ...
      case "Arrow Storm": {
        if (_coin - card.cost >= 0 && _pops - card.popcost >= 0) {
          ...
          let dice = Math.floor(this._rollDice());
  ...
  _renderCardDeck() {
    let spacer = <View style={styles.spacer}></View>
    return cards
      .map((item) => {
          ...
          <SlidingUpPanel draggableRange={draggableRange} ref={c => this._panel = c}>
            ...
            <View style={styles.panel}>
              ...
              <View style={styles.cardcontainer}>
                ...
                <ScrollView contentContainerStyle={{padding: 5, justifyContent: 'space-between'}} style={styles.carddeck} horizontal={true}>
